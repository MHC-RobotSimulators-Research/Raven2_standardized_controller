...
class ambf_raven:
    ...
    # Some stuff I copy pasted from Seans Dev branch
    def manual_move(self, arm, x, y, z, gangle):
        '''
        moves the robot based on cartesian coordinates
        '''
        curr_jp = np.array(self.arms[arm].get_all_joint_pos(), dtype="float")
        curr_tm = fk.fwd_kinematics(arm, curr_jp)
        curr_tm_test = fk.fwd_kinematics_p5(arm, curr_jp)
        print("initial p6 ", curr_tm)
        print("initial p5 ", curr_tm_test)
        curr_tm[0, 3] += x
        curr_tm[1, 3] += y
        curr_tm[2, 3] += z
        # print("modified pos ", curr_tm)
    ...
    def move_now(self, arm):
        ...
        Args:
            arm (int): integer specifying which arm of the raven robot will be moved
        """
